...
object Fetch : Selector {
    ...
    private var EMPTY = Target.Container()

    private fun Context.getTargetContainer(id: String): Target.Container {
        return ketherScriptContext?.rootFrame()?.rootVariables()?.get<Target.Container>(id)?.orElseGet { EMPTY } ?: EMPTY
    }
    ...
    override fun check(data: Selector.Data): CompletableFuture<Void> {
        ...
        return createAwaitVoidFuture {
            catchRunning {
                data.context.getTargetContainer(id).forEach {
                    data.container += it
                }
            }
        }
